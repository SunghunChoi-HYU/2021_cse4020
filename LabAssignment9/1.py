...
def key_callback(window, key, scancode, action, mods):
    ...
    if action==glfw.PRESS or action==glfw.REPEAT:
        if key==glfw.KEY_A:
            newM = [[np.cos(np.radians(10)), -np.sin(np.radians(10)), 0], [np.sin(np.radians(10)), np.cos(np.radians(10)), 0], [0,0,1]]
            gComposedM[:3,:3] = newM @ gComposedM[:3,:3]
        elif key==glfw.KEY_Z:
            newM = [[np.cos(np.radians(-10)), -np.sin(np.radians(-10)), 0], [np.sin(np.radians(-10)), np.cos(np.radians(-10)), 0], [0,0,1]]
            gComposedM[:3,:3] = newM @ gComposedM[:3,:3]
        elif key==glfw.KEY_S:
            newM = [[1,0,0],[0, np.cos(np.radians(10)), -np.sin(np.radians(10))], [0, np.sin(np.radians(10)), np.cos(np.radians(10))]]
            gComposedM[:3,:3] = newM @ gComposedM[:3,:3]
        elif key==glfw.KEY_X:
            newM = [[1,0,0],[0,np.cos(np.radians(-10)), -np.sin(np.radians(-10))], [0, np.sin(np.radians(-10)), np.cos(np.radians(-10))]]
            gComposedM[:3,:3] = newM @ gComposedM[:3,:3]
